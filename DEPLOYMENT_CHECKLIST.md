...
## 🖥️ Single-PC Lab Run (One Icon Launch)

Use these files from the project root to run the system like a desktop app:

- `launch_lab.bat` → starts server and opens browser automatically
- `launch_lab.vbs` → silent launcher (best target for desktop icon)
- `create_lab_shortcut.ps1` → creates desktop shortcut `ExamHall`

### Steps (Windows)
1. Open PowerShell in project folder.
2. Run:
    ```powershell
    powershell -ExecutionPolicy Bypass -File .\create_lab_shortcut.ps1
    ```
3. Double-click `ExamHall` desktop icon.

Notes:
- Keep the `ExamHall Server` terminal window open while using the app.
- Local URL opens automatically: `http://127.0.0.1:8000/`

---

## 🌍 Cloudflare Access for Other Devices
...
- Run:
   ...
   ```
- A temporary `https://...trycloudflare.com` link appears in tunnel window.
- Share this link with users on other devices.

### Option B: Custom Project Name URL (Recommended)
Use this when you want URL like `https://examhall.yourdomain.com`.

Prerequisites:
- Domain is added in Cloudflare account.
...
Run named tunnel:
```bat
start_cloudflare_named.bat examhall yourdomain.com
```

This creates URL format:
- `https://<projectname>.<yourdomain>`
- Example: `https://examhall.college.edu`
